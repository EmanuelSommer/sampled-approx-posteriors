...
from blackjax.base import AdaptationAlgorithm, ArrayLikeTree
from blackjax.diagnostics import effective_sample_size
from blackjax.util import pytree_size, streaming_average_update
from jax.flatten_util import ravel_pytree

from src.sai.training.callbacks import progress_bar_scan, save_position
...
def custom_window_adaptation(
    ...
    **extra_parameters,
    ...
    Args:
        ...
        logdensity_fn: The log density probability density function
        ...
        target_acceptance_rate: The acceptance rate that we target during step size
            adaptation.
        progress_bar: Whether we should display a progress bar.
        saving_path: The path to save the warmup positions for each state.
        **extra_parameters: The extra parameters to pass to the algorithm,
        e.g. the number of integration steps for HMC.
    """
    ...
    def one_step(carry, xs, device_id):
        n, rng_key, adaptation_stage = xs
    ...
    def run(
        ...
        if progress_bar:
            one_step_ = jax.jit(
                ...
                )(partial(one_step, device_id=device_id))
            )
        else:
            one_step_ = jax.jit(partial(one_step, device_id=device_id))
...
def mclmc_find_L_and_step_size(
    ...
    Args:
        ...
        tune1_steps: The number of steps for the first step of the adaptation.
        tune2_steps: The number of steps for the second step of the adaptation.
        tune3_steps: The number of steps for the third step of the adaptation.
        step_size_init: The initial step size for the MCMC algorithm.
        desired_energy_var_start: The desired energy variance for the MCMC algorithm.
        desired_energy_var_end: The desired energy variance for the MCMC algorithm
            at the end of the linear decay schedule. If the value is the same as
            the start value, no decay is applied.
...
def make_L_step_size_adaptation(
    ...
    """Adapt the stepsize and L of the MCLMC kernel. Designed for unadjusted MCLMC."""
    ...
    def predictor(
        ...
        """Do one step with the dynamics and update the prediction for the opt. stepsize.
...
def custom_mclmc_warmup(
    ...
    Args:
        logdensity_fn: The log density probability density function from
            which we wish to sample.
        diagonal_preconditioning: Whether we should use diagonal preconditioning.
        desired_energy_var_start: The desired energy variance at the start of the
            linear decay schedule.
        desired_energy_var_end: The desired energy variance at the end of the linear
            decay schedule. If the value is the same as the start value,
            no decay is applied.
        trust_in_estimate: The trust in the estimate.
        num_effective_samples: The number of effective samples.
        step_size_init: The initial step size for the MCMC algorithm.
    """
    diagonal_preconditioning = diagonal_preconditioning
